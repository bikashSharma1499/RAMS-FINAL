...
const LegalityProcess = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
       // const code = sessionStorage.getItem('agcLegalityCode');
        // if (code) {
        //     setAgreementCode(1); // Pass the retrieved code to setAgreementCode
        // }
        setAgreementCode(1);
    };
  ...
  // Trigger progress bar and PDF generation after lease data is fetched
  useEffect(() => {
    if (leaseData) {
      startProgress();
...
<Container className="mt-5">
      <h3>Please Wait while we process your document.... </h3>
      <ProgressBar now={progress} label={`${progress}%`} animated />
    </Container>
    <div className='legal d-none'>
      <div ref={legalRef}>
        <div className='p1'>
          <div className='stmp'>
            <h2>LEASE DEED</h2>
...
export default LegalityProcess;